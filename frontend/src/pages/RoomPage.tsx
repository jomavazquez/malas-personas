...
export const RoomPage = () => {
  ...
  useEffect(() => {
    ...
    socket.on("room:playerJoined", ({ userId, username }: { userId: string; username: string }) => {
      setPlayers((prev) => prev.find((p) => p.userId === userId) ? prev : [...prev, { userId, username, score: 0, isGuest: false, isJudge: false }]);
    });
  ...
  if (error) {
    return (
      <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16, fontFamily: F.body }}>
        ...
        <button onClick={() => navigate("/lobby")} style={{ background: C.base, color: "#fff", borderRadius: 12, padding: "12px 24px", fontFamily: F.display, fontWeight: 700, border: "none", cursor: "pointer" }}>← Volver</button>
      </div>
    );
  }
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {players.map((p, i) => (
                <div key={p.userId} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 999, background: p.userId === user?.id ? C.accent : C.base, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: F.display, fontWeight: 800, fontSize: 14, color: p.userId === user?.id ? C.base : "#fff" }}>
                    {p.username[0].toUpperCase()}
                  </div>
                  <span style={{ flex: 1, fontFamily: F.body, fontSize: 15, color: C.base }}>{p.username}</span>
                  <div style={{ display: "flex", gap: 6 }}>
                    {p.userId === user?.id && <span style={{ fontFamily: F.display, fontWeight: 700, fontSize: 11, letterSpacing: "0.05em", textTransform: "uppercase", color: C.accent }}>{t("room.you")}</span>}
